...
class DataLoader:
    ...
    def load(self, set_type=None, audio_type=None, generator=True,
             **load_kwargs):
        """
        Args:
        ...
        for set_type in set_types:
            pbar = self._call(set_type=set_type, audio_type=audio_type, **load_kwargs)
    ...
    def get_ret(self, obj, audio_type=DataRegister.PATH, **kwargs) -> dict:
...
class DataSaver:
    ...
    def save(self, data, set_type=DataRegister.FULL, audio_type=DataRegister.PATH, **kwargs):
        """
        ...
        Args:
            ...
            audio_type(DataRegister): `DataRegister.PATH` or `DataRegister.ARRAY`
                PATH -> a str of audio abs path
                audio -> a np.ndarray of audio, read from cv2, as (h, w, c)
        ...
        for iter_data, set_type in zip(data, set_types):
            ...
            self._call(iter_data, set_type=set_type, audio_type=audio_type, **kwargs)
    ...
    def parse_ret(self, ret, audio_type=DataRegister.PATH, **get_kwargs):
        raise NotImplementedError
...
class DatasetGenerator:
    ...
    def __init__(self, data_dir=None, audio_dir=None, label_dir=None, verbose=True, stdout_method=print):
        self.data_dir = data_dir
    ...
    def _gen_sets(
        ...
        Args:
            ...
            id_distinguish:
                the audio file name where having same id must not be split to different sets.
                e.g. id_distinguish = '_', the fmt of audio file name would like '{id}_{sub_id}.png'
